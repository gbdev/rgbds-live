...
for(var key in gameboy_hardware_constants)
{
    ...
    if (constant.description)
        doc = constant.description.replace("\n", "<br>");
    if (constant.value)
        doc += "<br><b>Value:</b> " + constant.value;
    if (constant.flags)
    {
    ...
    if (constant.alias)
        doc += "<br><b>Alias:</b> " + constant.alias;
    var i = {
        caption: key, value: key, meta: "",
        docHTML: doc,
    };
...
var gbz80Completer = {
    getCompletions: function(editor, session, pos, prefix, callback) {
        ...
        if (before.trim() == "" || before.trim().endsWith(":")) {
            ...
            callback(null, gbz80CompleterConstants.filter(function(c) {
