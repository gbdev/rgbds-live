...
class TrackerUI {
  ...
  constructor() {
    ...
    document.getElementById("trackerEffectType").onchange = (e) => {
      this.setEffectType(
        e.target.selectedIndex > 0 ? e.target.selectedIndex - 1 : null,
      );
    };
    ...
    tracker.onkeydown = (e) => {
      ...
      if (this.selected_type == "note" || this.selected_type == "instrument") {
        ...
        if (this.selected_type == "instrument" && e.code == "Delete")
  ...
  setNote(value) {
    var cell =
      song.patterns[this.pattern_index][this.selected_row][this.selected_col];
    cell.note = value;
    this.getCell(this.selected_row, this.selected_col, "note").innerText =
      noteToText(value);
  ...
  setInstrument(value) {
    song.patterns[this.pattern_index][this.selected_row][
      this.selected_col
    ].instrument = value;
  ...
  setEffectType(value) {
    ...
    this.getCell(this.selected_row, this.selected_col, "effect").innerText =
      effectToText(c.effectcode, c.effectparam);
    this.setSelection(this.selected_row, this.selected_col, "effect");
    player.updateRom();
  }
  ...
  setSelection(row, col, type) {
    ...
    if (type != "effect") {
      ...
      if (c.effectcode == null) {
        ...
        document.getElementById("trackerEffectType").selectedIndex =
          c.effectcode + 1;
        document.getElementById("trackerEffectNibbleLow").value =
          c.effectparam & 0x0f;
        document.getElementById("trackerEffectNibbleHigh").value =
          (c.effectparam & 0xf0) >> 4;
        document.getElementById("trackerEffectByte").value = c.effectparam;
        document.getElementById("trackerEffectWaveform").selectedIndex =
          c.effectparam >> 4;
        document.getElementById("trackerEffectPanningLD1").checked =
          c.effectparam & 0x01;
        document.getElementById("trackerEffectPanningLD2").checked =
          c.effectparam & 0x02;
        document.getElementById("trackerEffectPanningLW").checked =
          c.effectparam & 0x04;
        document.getElementById("trackerEffectPanningLN").checked =
          c.effectparam & 0x08;
        document.getElementById("trackerEffectPanningRD1").checked =
          c.effectparam & 0x10;
        document.getElementById("trackerEffectPanningRD2").checked =
          c.effectparam & 0x20;
        document.getElementById("trackerEffectPanningRW").checked =
          c.effectparam & 0x40;
        document.getElementById("trackerEffectPanningRN").checked =
          c.effectparam & 0x80;
        document.getElementById("trackerEffectDutyCycle").selectedIndex =
          c.effectparam >> 6;
      }
      document.getElementById("trackerEffectNibbleLow").style.display = [
        ...
        ? ""
        : "none";
      document.getElementById("trackerEffectNibbleHigh").style.display = [
        0, 5, 10,
      ].includes(c.effectcode)
        ? ""
        : "none";
      document.getElementById("trackerEffectByte").style.display = [
        ...
        ? ""
        : "none";
      document.getElementById("trackerEffectWaveform").style.display = [
        4,
      ].includes(c.effectcode)
        ? ""
        : "none";
      document.getElementById("trackerEffectPanningTable").style.display = [
        8,
      ].includes(c.effectcode)
        ? ""
        : "none";
      document.getElementById("trackerEffectDutyCycle").style.display = [
        9,
      ].includes(c.effectcode)
        ? ""
        : "none";
  ...
  updateEffectInfo() {
    ...
    else if (c.effectcode === 3)
      info = `Tone portamento by ${c.effectparam} units`;
    ...
    else if (c.effectcode === 12)
      info = `Set volume to ${c.effectparam & 0x0f}/15`;
    else if (c.effectcode === 13)
      info = `Jump to row ${c.effectparam} on the next pattern`;
    else if (c.effectcode === 14)
      info = `Cut note after ${c.effectparam} ticks`;
  ...
  loadPattern(index) {
    ...
    for (var index = 0; index < pattern.length; index++) {
      ...
      for (var track = 0; track < 4; track++) {
        ...
        cell_node.children[1].innerText = instrumentNumberToText(
          cell.instrument,
        );
        cell_node.children[2].innerText = effectToText(
          cell.effectcode,
          cell.effectparam,
        );
      }
